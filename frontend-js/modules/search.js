...
export default class Search {
    // 1. Select DOM elements & track data
    constructor(){
    ...
    renderResultsHTML(posts) {
        if(posts.length){
            ...
            ${posts.map((post)=>{
                ...
                <img class="avatar-tiny" src="${post.author.gravatar}"> <strong>${post.title}</strong>
                <span class="text-muted small">by ${post.author.username} on ${postDate.getMonth()+1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
              </a>`
            }).join('')}
            </div>`)
        } else {
            this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry! We could not find any results for that serach.</p>`
        }
        this.hideLoaderIcon()
